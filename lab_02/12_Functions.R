...
net_salary <- function(base_salary, allowance, working_days, overtime_hours){
  
  salary_per_day <- base_salary / 26
  
  salary_per_hour <- salary_per_day / 8
  
  overtime_pay <- overtime_hours * salary_per_hour * 1.5
  
  total_salary <- salary_per_day * working_days + allowance + overtime_pay
  
  tax <- total_salary * 0.1
  
  net <- total_salary - tax
  
  return(net)
}

# Test
net_salary(10000000,1000000,24,10)
