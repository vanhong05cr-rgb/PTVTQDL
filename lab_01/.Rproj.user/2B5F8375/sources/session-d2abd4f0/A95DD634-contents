...
sat2<-data.frame(IdClient = c(1,2,5,9), Region = c(("TP.HCM", 3), "Dong Nai"))
set1
set2
...
# Sắp xếp dữ liệu 
sort(set1$IdClient)
sort(set1$IdClient, decreasing=TRUE)# Trả về value
order(set1$IdClient, decreasing=TRUE)# Trả về index

# Sử dụng hàm order để lấy chỉ số index đã sắp xếp sau đó áp dụng cho dataframe
set1[set1$IdClient, decreasing=TRUE,]
...
iris[iris$Species=="setosa" & iris$Petal.Lenght >=1.5,]
